...
import { useGetItemProgressQuery, useUpdateItemProgressMutation } from '@/redux/api/progressApiSlice';
...
const CoursePlayer = ({ itemId, itemType, onDoneChange }) => {
    ...
    const { data: itemProgress, isLoading: isProgressLoading } = useGetItemProgressQuery(
        { courseId: item?.courseId, sectionId: item?.sectionId, itemId: item?._id },
    ...
    useEffect(() => {
        ...
        const handleSaveProgress = async () => {
            const watchedSeconds = currentVideo.getCurrentTime?.() || itemProgress?.watchedSeconds || 0;
            if (watchedSeconds < (itemProgress?.watchedSeconds || 0)) return;
            const totalSeconds = currentVideo.getDuration?.() || itemProgress?.totalSeconds || 0;
        ...
        interval = setInterval(() => {
            ...
            const progressPercent = totalSeconds ? Math.round((watchedSeconds / totalSeconds) * 100) : 0;
            if (!hasSavedCompletion && progressPercent >= 85 && watchedSeconds >= (itemProgress?.watchedSeconds || 0)) {
    ...
    if (
        (isItemLoading && !item) ||
        (isProgressLoading && !itemProgress)
    ) {
        return (
    ...
    return (
        <div className='flex flex-col min-h-full'>
            <div className="flex-grow">
                {itemType === "Lecture" ? item.type === "article" ? (
                    <ArticleRender item={item} itemProgress={itemProgress} setIsDone={setIsDone} />
                ) : (
                    <div className="flex flex-col">
                        ...
                        <div className=" mx-12 mt-8 md:mx-12 md:mt-12 lg:mx-20 lg:mt-12 overflow-auto">
                            <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                                ...
                                <button className="flex-shrink-0 rounded-md bg-blue-100 hover:bg-blue-200 px-3 py-2 flex items-center justify-center">
...
const ArticleRender = ({ item, itemProgress, setIsDone }) => {
    ...
    const saveProgress = async (progressPercentToSave) => {
        ...
        try {
            ...
            if ( progressPercentToSave >= 85 && progressPercentToSave > itemProgress?.progressPercent ) {
    ...
    useEffect(() => {
        ...
        if (progressPercent > 0 && container.scrollHeight > container.clientHeight) {
            const scrollTop =
                ((container.scrollHeight - container.clientHeight) * progressPercent) / 100;
