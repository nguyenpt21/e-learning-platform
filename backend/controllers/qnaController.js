...
export const solveTheQnA = async (req, res) => {
  try {
    const { qnaId } = req.params;
    ...
    if (qna.author.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ message: "You do not have permission to solve this QnA" });
    }
    const comment = qna.comments.id(commentId);
    if (!comment) {
      return res.status(404).json({ message: "Comment not found" });
    ...
    qna.isSolved = true;
    await qna.save();
    res.status(200).json({ message: "QnA solved successfully" });
  } catch (error) {
    console.log("Error solving the QnA:", error);
    res.status(500).json({ message: "Failed to solve the QnA" });
  }
};
